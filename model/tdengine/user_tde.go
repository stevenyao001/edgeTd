...
func (u *UserTd)Find() error {
	...
	sqlsGK := fmt.Sprintf("select instant_ep, num, status from megnet_gk_001 limit 10")
	...
	for rowsGK.Next() {
		tmp := UserTdGK{}
		err = rowsGK.Scan(&tmp.InstantEp, &tmp.Num, &tmp.Status)
		if err != nil {
			logger.ErrorLog("UserTde-Find", "赋值报错", "", err)
			return err
		}
...
func (u *UserTd)Insert(data *Input) error {
	...
	sqlsGK := fmt.Sprintf("insert into megnet_gk_001 (ts, megnet_id, instant_ep, num, status) values (%d, %d, %d, %d, %d)", tsGK, ts, data.Properties.InstantEp, data.Properties.Num, data.Properties.Status)
